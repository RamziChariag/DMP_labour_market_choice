...
CONTAINS
  ...
  subroutine objective_function(theta,p,obj_fun,simmoments)
    IMPLICIT NONE
    REAL(wp) :: obj_fun
...
!!$            / GLOBAL_TRUE_SD_MATRIX)**2 )
       ...
       DMoments(56:60) = DMoments(56:60) * 4.0_wp * 1000.0_wp ! only E2U after 15 years
       DMoments(61:75)  = 0.0_wp
       DMoments(76:80) = DMoments(76:80) * 4.0_wp * 1000.0_wp ! only J2J after 15 years

       DMoments(81:100) =  DMoments(81:100)  ! age profile for wages
...
!!$       
!!$       obj_fun = -0.5_wp * sum( (tmp_array)**2 )
    ...
    ELSE
       ...
       obj_fun = -999999999.0_wp
    END IF
    
    ! check if trying to return either nan or infinite objective function
    IF ( isnan(obj_fun) ) THEN
       obj_fun = -888888888.0_wp
    ELSE IF ( ABS(obj_fun) .GT. HUGE(obj_fun) ) THEN
       obj_fun = -888888888.0_wp
    END IF
