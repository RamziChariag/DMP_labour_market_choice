...
contains
  subroutine solveModel(m,p,e)
    ...
    !-------- SET PLANNER'S MATCHING INDICATOR
    m_old%M_planner_xy = 0.0_wp
    
    do i=1,p%gridSizeWorkers
       do j=1,p%gridSizeFirms
          
          if ( (p%xvals_x(i) .ge. p%planner1 + p%planner3*p%yvals_y(j) &
               + p%planner5*p%yvals_y(j)**2 + p%planner7*p%yvals_y(j)**3 &
               + p%planner9*p%yvals_y(j)**4 + p%planner11*p%yvals_y(j)**5 &
               ) &
               .and. ( p%yvals_y(j) .ge. p%planner2 + p%planner4*p%xvals_x(i) &
               + p%planner6*p%xvals_x(i)**2 + p%planner8*p%xvals_x(i)**3  &
               + p%planner10*p%xvals_x(i)**4 + p%planner12*p%xvals_x(i)**5) &
               ) then
             m_old%M_planner_xy(i,j) = 1.0_wp
    ...
    m_new%M_planner_xy = m_old%M_planner_xy
	...
	!    call printMat(benefit_xy)
        ...
        do i = 1, 2000  ! If change this it needs to be changed below when setting converged
           !    print *, 'Iteration: '
           !    print '(i5)', (i)
           ...
           Snew_xy = discounting * (m_old%f_xy &                ! flow output
                ...
                + m_old%S_xy ) &
                * m_new%M_planner_xy ! Only include matches the planner wants

           m_new%S_xy = ( (p%relax_s) * m_old%S_xy + (1.0_wp - p%relax_s) * Snew_xy )* m_new%M_planner_xy &
                + (m_new%M_planner_xy - 1.0_wp) * 1000.0_wp
