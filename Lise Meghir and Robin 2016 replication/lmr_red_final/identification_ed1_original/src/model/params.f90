module params
!*FD This module defines the parameters that will be used in solving the model
...
contains
  subroutine InitExogenousParameters(p)
    ...
    p%s0   =   1.0_wp;
    p%s1   =   0.10_wp;
    p%zeta =   0.008_wp;  ! expected working life of 40 years (480 months)

    ! Effective discount rates
    p%r     = 0.05_wp/12.0_wp;     ! discount rate of 5% per year
    p%delta = 0.08_wp; ! 4.0_wp/12.0_wp;        ! Poisson probability of productivity shock (expect 1 per quarter)
    p%chi   = 0.025_wp/12.0_wp     ! mortality rate (40 year working life)
    ...
    p%alpha = 0.75_wp;        ! parameter multiplying matching function

    ! Policy parameter
    p%tau   = 0.0_wp;         ! Firing tax

    ! Worker bargaining power
    p%beta  = 0.5_wp;        ! Bargaing power of workers

    ! flow cost and benefit of firm and worker
    p%c     = 0.10_wp;        ! job posting cost
    p%b     = 0.30_wp;        ! home production, produce as if matched to 0.1 quantile of firm distn
...
!!$    write(*,*) 'REDUCED GRID FOR DEBUGGING'
    ...
    !Size of labour force and potential number of firms
    p%nWorkers     =  10000 !25000;       ! Size of the labour force
    p%nFirms       =  10000 !50000;       ! Dummy number of firms
