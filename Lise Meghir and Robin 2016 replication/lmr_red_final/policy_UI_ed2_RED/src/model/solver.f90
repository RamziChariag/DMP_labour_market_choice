...
contains
  subroutine solveModel(m,p,e)
    ...
    real(wp), allocatable :: h_x(:) ! integrate over y
    real(wp), allocatable :: f_bar_x(:,:) ! expected output of type x worker
    ...
    allocate(h_x(p%gridSizeWorkers))
    allocate(f_bar_x(p%gridSizeWorkers,p%gridSizeFirms))
    ...
    !##################################################
     ...
     do j = 1, 2000
        ...
        do i = 1, 1000  ! If change this it needs to be changed below when setting converged
           ...
           ! --- 
           ! Calculate benefit rates consistent with current taxes
           
           if (i > 1) then
              mrkt_prod = sum( m_old%f_xy * m_old%h_xy )
              
              ! create average output per worker
              f_bar_x = 0.0_wp
              h_x = 0.0_wp
              do xx=1,p%gridSizeWorkers
                 do yy=1,p%gridSizeFirms
                    h_x(xx) = h_x(xx) +  m_old%h_xy(xx,yy)
                    f_bar_x(xx,:) = f_bar_x(xx,:) + m_old%f_xy(xx,yy)
                 end do
                 f_bar_x(xx,:) = f_bar_x(xx,:) / h_x(xx) 
              end do

              ! home_production here is unemployent distn weighted average output
              home_prod = sum( f_bar_x(:,1) * m_old%u_x )
              p%benefit_UI = p%tau * mrkt_prod / home_prod

!!$              ! for debugging
!!$              write(*,*) "mrkt_prod", mrkt_prod
!!$              write(*,*) "home prod", home_prod
!!$              write(*,*) "beneift rate", p%benefit_UI
!!$              write(*,*) "tax rate", p%tau
           ...
           !---------------------------------------------------
           r_chi_WorkerOppCost_xy = p%benefit_UI*f_bar_x + benefit_xy &
                +  p%s0 * kappa * p%beta &
           ...
           Snew_xy = discounting * (( 1.0_wp - p%tau ) * m_old%f_xy &                ! flow output
