...
program MPI_MSW
  ...
  open(13,file='data/mom_ed1.raw')
  read(13,*) (GLOBAL_TRUE_MOMENT_MATRIX(i),i=1,nmoments)
  close(13)
  
  open(13,file='data/se_ed1.raw',form='formatted')
...
!!$  global_merror = exp( global_merror ) 
  ...
  !  call readFromCSV('data/Choleski_shrink_ed2.raw', GLOBAL_TRUE_CHOLESKY_MATRIX )
  write(*,*) 'Finished reading in data moments and covariance matrix ... '
  
  ! INIT MPI
  call MpiMCMCInit()
  
  optimizer_params%CR = 0.75_wpfmpi      ! 25% prob of 2 coeff changing.
  optimizer_params%parameter_size = N
  optimizer_params%chain_count    = 95   ! number of parrallel chains
  optimizer_params%chain_length   = 500  ! length of chain
...
!  optimizer_params%max_iteration  = 1000     ! number of elements in the differ  
 optimizer_params%max_iteration  = 10000     ! number of elements in the differ
  ...
  optimizer_params%stat_display_rate = 500   ! frequency of display
...
!!$  optimizer_params%shock_add_std  = 0.00001_wpfmpi
  optimizer_params%initial_posterior_value = -10000000.0_wpfmpi
