...
foreach ed of numlist 1/2 {
	...
	*matrix se_`ed'_ = se_`ed''
	...
	foreach ed of numlist 1/2 {
		file open myfile_`ed' using "moments/mom_ed`ed'_no_growth.raw", write replace
	...
	foreach ed of numlist 1/2 {
		file open myfile_se_`ed' using "moments/se_ed`ed'_no_growth.raw", write replace
		foreach moment of numlist 1/12 {
			foreach year of numlist 1/20 {
				file write myfile_se_`ed' %24.16f (se_`ed'_`moment'[`year']) %3s " "
			}
		}
		file write myfile_se_`ed' %24.16f (0.054)
		file close myfile_se_`ed'
	}
