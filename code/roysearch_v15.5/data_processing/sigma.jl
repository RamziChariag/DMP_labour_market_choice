...
# The per-moment sampling-variance vector (Stage 8, sampling_var_{window}.csv)
# is computed entirely from closed forms — NO bootstrap, no resampling, no RNG.
# It is therefore a deterministic function of the data.
...
#
# Every moment uses a CLOSED FORM (no resampling): CPS shares get the Kish
# weighted-proportion variance; ASEC wage moments get analytic delta-method /
# order-statistic forms on the pooled window cross-section, each divided by the
# Kish N_eff of the relevant sector pool; flow hazards and θ use the delta
# method; SIPP moments are already sampling variances and are copied from the
# Σ̂ diagonal construction. All helpers below are pure functions of the data.

# Exact analytic sampling variance of a weighted proportion p̂ = Σ_numer w / Σ_denom w.
# This is the SAME 1/N footing the weighted-share bootstrap approximated, but O(N)
# rather than O(B·N): the Kish effective sample size of the DENOMINATOR pool gives the
# binomial-proportion variance  Var(p̂) = p̂·(1−p̂)/N_eff,  N_eff = (Σ_denom w)²/Σ_denom w².
# Matches the SIPP delta-method diagonal form. `denom`/`numer` are Bool masks over the
# pooled rows (identical numerator/denominator populations to moments.jl's weighted ratio).
...
function compute_influence_functions_and_sigma_full()
    ...
    for (wname, wdef) in WINDOWS
        ...
        #
        # FULLY CLOSED-FORM — NO resampling anywhere: every entry has an analytic
        # 1/N sampling variance (Kish proportion variance for CPS shares; analytic
        # delta-method / order-statistic forms for ASEC wage moments; delta method
        # for hazards and θ; SIPP diagonals copied). The block is deterministic:
        ...
        #     numerator/denominator masks moments.jl uses. This replaces the former
        #     weighted bootstrap (which resampled the full multi-million-row window
        #     B times → O(B·N) and froze on base_fc); _kish_prop_var is O(N) and
        #     gives the same 1/N sampling-variance footing. Pooled arrays and masks
        #     built once.
        ...
        svar[:ur_total]      = _kish_prop_var(cw, lfx, lfx .& unemp)
        svar[:ur_U]          = _kish_prop_var(cw, lfx .& .!skl, lfx .& .!skl .& unemp)
        svar[:ur_S]          = _kish_prop_var(cw, inlf .& skl, inlf .& skl .& unemp)
        svar[:skilled_share] = _kish_prop_var(cw, lfx, lfx .& skl)
        ...
        ts_var_raw = _kish_prop_var(cw, trues(nrow(cps_w)), intr .& .!inlf)
        ...
        if hasproperty(cps_w, :DURUNEMP)
            ...
            svar[:ltu_share_S] = _kish_prop_var(cw, skl_u, ltu)
        else
            svar[:ltu_share_S] = NaN
        end

        # (b) ASEC wage moments — analytic 1/N sampling variances (NO bootstrap).
        #     The Σ̂ construction above builds only PER-YEAR across-period ψ
        #     deviations (the time-dispersion footing), not per-observation
        #     influence contributions, so the within-sample sampling variance is
        #     computed here from closed forms on the pooled ASEC log-wage sample.
        #     Every form divides by the Kish N_eff of the relevant sector pool.
        ...
        neff_u = _kish_neff(wu); neff_s = _kish_neff(ws)
        ...
        # pool (Silverman bandwidth, Kish N_eff — the same estimator used for the
        # Σ̂ median influence functions).
