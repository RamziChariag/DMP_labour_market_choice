############################################################
# code/smm_fct_ps/main.jl — SMM estimation entry point (PS(x) variant)
#
# Usage (from project root):
#   julia --threads auto code/smm_fct_ps/main.jl
#
# Project layout:
#   code/
...
const SOLVER_DIR   = joinpath(SMM_DIR, "..", "solver_fct_ps")
...
if WINDOW in (:crisis_fc, :crisis_covid)
    ...
    baseline_jls = joinpath(SMM_OUT_DIR, "smm_result_base_fc$(W_SUFFIX)_fct_ps.jls")
    ...
    _warmstart_jls = joinpath(SMM_OUT_DIR, "smm_result_base_fc$(W_SUFFIX)_fct_ps.jls")
...
run_params = SMMRunParams(
    ...
    sa_max_reheats     = 0,       # cap on total reheats (0 = unlimited)
...
save_results(results, joinpath(TABLES_DIR, "smm_estimates_$(WINDOW)$(W_SUFFIX)_fct_ps.csv"))
...
smm_jls_path = joinpath(SMM_OUT_DIR, "smm_result_$(WINDOW)$(W_SUFFIX)_fct_ps.jls")
