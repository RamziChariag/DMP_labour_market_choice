...
function solve_model!(model::Model)::SolveResult
    ...
    streak = 0
    gB = sim.global_B;  gK = sim.global_K
    global_resid = Vector{Float64}(undef, sim.maxit_global)
    ...
    for it in 1:sim.maxit_global
        ...
        # Stall-out: after the burn-in, if the residual has not beaten its
        # best-so-far over the last gK iterations, the coupling is limit-cycling.
        if gB > 0 && it > gB + gK && minimum(@view global_resid[it-gK:it]) >= minimum(@view global_resid[1:it-gK])
            sim.verbose >= 1 && @printf("[global]  stalled it=%d  d=%.3e — no improvement over %d iters, stopping\n", it, d, gK)
            break
        end
