...
#
# NOT CALLED by the two-stage run below, which polishes with DE instead: at 25
# parameters the simplex stalls with its best vertex fixed. These settings and the
# method = :neldermead path are retained and working, so a run can be pointed back
# at them by swapping the stage-2 call.
# ============================================================
...
#
# DE_LOCAL_K builds the initial population by perturbing that many coordinates of the
# SA optimum, each at DE_LOCAL_SIGMA times its own ΔQ<1 width, instead of drawing
# uniformly over the box.  Measured useful-draw rate 0.51 at k=3 against 0.08 at k=25
# and ~0 for the box draw: the loss with density is not infeasibility (which barely
# moves) but the compounding of many small increases in Q.
#
# de_cr stays high.  The population's sparsity already caps proposal density — a k=3
# member differs from the seed in 3 coordinates, so b−c is nonzero in at most 6 — and
# CR's job is recombining members into points between them.
# ============================================================
...
print_spec(spec; sa_subset_k = SA_SUBSET_K, de_local_k = DE_LOCAL_K,
           de_local_sigma = DE_LOCAL_SIGMA)
...
# Stage 2: population search in the basin stage 1 landed in.  Differential evolution
# rather than Nelder-Mead: at 25 parameters the simplex stalls with its best vertex
# fixed while its search direction turns orthogonal to descent, and the run log shows
# exactly that.  DE's proposal a + f·(b−c) draws its direction and length from the
# population, so it moves several coordinates jointly at scales the population
# carries — which is what this objective needs and what a simplex cannot build.
res_pol = run_smm(_spec_with_init(spec, res.theta_opt); method = :de,
                  de_local_k     = DE_LOCAL_K,
                  de_local_sigma = DE_LOCAL_SIGMA)
