...
Conditioning behaviour (controlled by `cond_target`):
  cond_target == 0.0  diagonal-σ: W = Diagonal(1 / σ̂²_samp) from the per-moment
                       ...
                       moments on one 1/N footing). NOT 1/diag(Σ̂) — the IF
                       diagonal mixes across-period and within-sample footings.
  cond_target == 1.0  compressed diagonal:  W_kk = log(1 + 1/σ_k²) from diag(Σ̂)
...
g_k = m_k − m̂_k in compute_loss_matrix (all scaling lives in W). It is a
separate file from sigma_{window}.csv, which the full-Σ⁻¹ path reads unchanged.
"""
function load_weight_matrix(;
    ...
    # per-moment sampling-variance vector (one 1/N footing). Reads its OWN file,
    # never diag(Σ̂). A missing file means the derived dir predates v11.7 — error
    ...
    if cond_target == 0.0
        ...
        isfile(sv_file) || error(
            ...
            "weight needs the per-moment sampling variances; run the v11.7 data pipeline first.")
...
function model_moments(obj)
    ...
    # Skilled EE-move mean wage step (SIPP moment; identifies β_S). Carried
    # through the objective but left untargeted while β_S is pinned — freeing
    # β_S in smm_main.jl FIX_PARAMS is what activates it. Guarded for older
