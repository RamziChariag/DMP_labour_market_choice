...
#   1. Data-vs-model panel  (output/plots/transition_panel_<sc><W>.png)
#   2. Model decomposition  (output/plots/model_transition_<sc><W>.png)
#
# Public API:
...
# Two modes, as transition.jl: included by transition_main.jl or standalone.
@isdefined(ROYSEARCH_PATHS_LOADED) || include(joinpath(@__DIR__, "..", "paths.jl"))
if !@isdefined(TRANSITION_DIR)
    const TRANSITION_DIR = joinpath(CODE_DIR, "transition")
end
