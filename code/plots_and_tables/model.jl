...
function fig_densities(obj)
    ...
    # Ability-marginal densities: the stationary masses are 2D on the (aU,aS)
    # copula grid, so reduce to the plotted ability axis.  W2[i,j] has i=aU,
    # j=aS: unskilled objects marginalise over aS (dims=2), skilled over aU
    # (dims=1).  eU_rowU and eS_totS are the pre-reduced marginals the object
    # already exports; mU/mS/uS are reduced here from the exported matrices.
    uU_x = vec(sum(obj.uU,     dims = 2))
    tU_x = vec(sum(obj.tU,     dims = 2))
    mU_x = vec(sum(obj.mU_mat, dims = 2))
    uS_a = vec(sum(obj.uS_mat, dims = 1))
    mS_a = vec(sum(obj.mS_mat, dims = 1))

    p1a = plot(xg, uU_x,       label=L"u_U(x)",           color=_C1)
    plot!(p1a, xg, tU_x,       label=L"t(x)",             color=_C2)
    plot!(p1a, xg, obj.eU_rowU,label=L"e_U(x)",           color=_C3)
    plot!(p1a, xg, mU_x,       label=L"m_U(x)",           color=:black, ls=:dash)
    title!(p1a, "Unskilled segment densities")
    xlabel!(p1a, L"x")
    ylabel!(p1a, "Density")

    p1b = plot(xg, uS_a,       label=L"u_S(x)",           color=_C1)
    plot!(p1b, xg, obj.eS_totS,label=L"e_S^{\rm tot}(x)", color=_C3)
    plot!(p1b, xg, mS_a,       label=L"m_S(x)",           color=:black, ls=:dash)
...
function fig_unskilled_values(obj)
    xg = obj.xg
    # τ(aU,aS) is a 2D region; collapse to the aS-averaged training fraction
    # over aU and mark the threshold aU where the majority trains.
    τ_x = vec(sum(obj.τ_mat, dims = 2)) ./ size(obj.τ_mat, 2)
    x_bar_idx = findfirst(τ_x .> 0.5)
    ...
    plot!(p2, xg, obj.Usearch,      label=L"U_U(x)=\max",         color=:black, ls=:dash, lw=2.2)
...
function fig_employment_heatmaps(obj; percentile_print = 1.00)
    ...
    pg_w  = pg[iS];   eS_w = obj.eS_pS[ix, iS]
...
function fig_training_policy(obj)
    # τ(aU,aS) is a 2D indicator region; plot the aS-averaged training
    # fraction as a function of aU (a 1D summary of the region).
    τ_x = vec(sum(obj.τ_mat, dims = 2)) ./ size(obj.τ_mat, 2)
    p4 = plot(obj.xg, τ_x,
              label=L"\bar{\tau}(a_U)", color=_C2, lw=2, legend=false)
    xlabel!(p4, L"a_U")
    ylabel!(p4, L"\bar{\tau}(a_U)")
    plot!(p4, yguidefontrotation=-90)
    ylims!(p4, -0.05, 1.15)
    p4
end

# Sorting plane on ability-rank axes: the joint ability density (copula mass)
# with the 2D training frontier τ(aU,aS) and the cross-market policy d(aU,aS)
# overlaid.  Because ρ_x < 0, the mass sits in the two off-diagonal corners;
# the training region is the upper-left (low aU, high aS), so skilled workers
# come from comparative — not absolute — advantage.  Axes are ranks F_ℓ so the
# copula is shown on its uniform margins.
function fig_sorting_plane(obj)
    FU = obj.Fell                                    # rank F_ℓ(aU) = F_ℓ(aS) (shared marginal)
    τ  = obj.τ_mat                                    # τ[i,j]: i=aU, j=aS
    Nx = obj.Nx

    # Copula density on ranks: divide joint mass by the marginal weights so the
    # heatmap shows c_ρ(F_U,F_S), not the node-spacing of the GL rule.
    dens = [obj.W2[i, j] / (obj.waU[i] * obj.waS[j]) for i in 1:Nx, j in 1:Nx]

    # Training frontier as a curve F_S*(F_U): the lowest aS-rank that trains at
    # each aU.  Columns with no training map to NaN (drawn as a gap).
    frontier = fill(NaN, Nx)
    for i in 1:Nx
        j = findfirst(>(0.5), @view τ[i, :])
        j !== nothing && (frontier[i] = FU[j])
    end

    fig = heatmap(FU, FU, dens',
                  xlabel = L"unskilled-ability rank $F_\ell(a_U)$",
                  ylabel = L"skilled-ability rank $F_\ell(a_S)$",
                  color = :inferno, legend = false, grid = false,
                  yguidefontrotation = -90)
    plot!(fig, FU, frontier, color = _C2, lw = 2.6, label = L"\tau(a_U,a_S)")

    # d(aU,aS) overlay: the aS-averaged cross-market policy along the bottom, so
    # the reader sees where trained workers redirect search to the U-market.
    d_row = vec(sum(obj.d, dims = 2)) ./ Nx
    plot!(fig, FU, d_row, color = :gold, lw = 2, ls = :dot, label = L"d(a_U,a_S)")

    annotate!(fig, 0.30, 0.80, text("train",          :white, 11))
    annotate!(fig, 0.68, 0.20, text("stay unskilled", :white, 11))
    xlims!(fig, 0.0, 1.0);  ylims!(fig, 0.0, 1.0)
    fig
end
...
function fig_unemployment_values(obj)
    ...
    plot!(p11, xg, obj.Usearch, label=L"U_U(x)", color=_C1, lw=2)
    xlabel!(p11, L"x")
    ylabel!(p11, "Value")
    p11
end
...
function fig_skill_premium(obj)
    ...
    # The realised wage in a cell is the OJS covered fraction s(p) of the poached-worker
    # surface wS1 plus the remainder on wS0 — the same split the moment layer applies, so
    # the figure shows the wage the moments are computed from.
    wS_actual = fill(NaN, obj.Nx, obj.NpS)
    for ix in 1:obj.Nx
        poj_ix = clamp01(obj.poj[ix])
        for jp in 1:obj.NpS
            s  = _soft_oj_weight(pg[jp], poj_ix, pg, jp, obj.NpS)
            w1 = obj.wS1_surface[ix, jp];  w0 = obj.wS0_surface[ix, jp]
            wS_actual[ix, jp] = s >= 1.0 ? w1 : (s <= 0.0 ? w0 : s * w1 + (1.0 - s) * w0)
        end
    end
...
function fig_skilled_employment_by_PS(obj, PS::Float64;
    ...
    PSg = PS .* xg                              # linear loading P_S(x) = P_S·x
...
function make_all_plots(obj; output_dir::String = "output/plots",
    ...
    figures = [
        ...
        ("fig04b_sorting_plane",       fig_sorting_plane(obj)),
