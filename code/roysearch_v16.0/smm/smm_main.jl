...
#
# The operative stopping rule: halt once the best Q has improved by less than
# NM_RATE_TOL over each of NM_RATE_RUNS consecutive windows of NM_RATE_WINDOW
# objective evaluations. Q is a chi-square, so the tolerance is an improvement
# budget in sampling-standard-error units — 0.05 is one twentieth of one moment
# moving by one standard error.
#
# Calibrated by replaying a full base_fc trace (11 800 evaluations, Q 1289.45 →
# 1275.38): these values stop at ~10 800 leaving 0.30 in Q, while 0.10 and above
# fire at 3 500 during a plateau the descent later escapes, discarding 10 units.
#
# A window with NO improvement resets the streak instead of counting toward it: a
# flat stretch is the simplex against an infeasible wall, which in 25 dimensions can
# persist for thousands of evaluations and then recover, so it needs patience rather
# than a stop.  Set NM_RATE_TOL = 0.0 to disable and fall back to nm_no_improve.
# ============================================================
NM_RATE_TOL    = _env_f64("ROYSEARCH_NM_RATE_TOL", 0.05)
NM_RATE_WINDOW = _env_int("ROYSEARCH_NM_RATE_WINDOW", 100)
NM_RATE_RUNS   = _env_int("ROYSEARCH_NM_RATE_RUNS", 3)
...
res = run_smm(spec; method = :sa, seed_bank = seed_bank, prev_optimum = prev_optimum,
        ...
        nm_rate_window = NM_RATE_WINDOW,
        nm_rate_runs   = NM_RATE_RUNS)
