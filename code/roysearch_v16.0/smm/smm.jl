...
function run_smm(
    ...
    #
    # Q is a chi-square, so ΔQ = 1 is one moment moving by one sampling standard
    # error; nm_rate_tol is therefore an improvement budget in those units, and at
    # measured solver noise of ΔQ ≈ 1e-6 the threshold is statistical, not numerical.
    # 0.05 per 100 evaluations over 3 consecutive windows was calibrated by replaying
    # a full base_fc trace (11 800 evaluations, 1289.45 → 1275.38): it stops at
    # ~10 800 leaving 0.30 in Q, while 0.10 and above fire at 3 500 during a plateau
    # the descent later escapes, discarding 10 units.
    #
    # A window with NO improvement at all resets the streak instead of counting it:
    # a flat stretch is the simplex against an infeasible wall, which in 25 dimensions
    # can persist for thousands of evaluations and then recover, so it needs patience
    # rather than a stop.
    nm_rate_tol    :: Float64 = 0.05,
    nm_rate_window :: Int     = 100,
    nm_rate_runs   :: Int     = 3,
) :: SMMResult
    ...
    elseif method in (:neldermead, :lbfgs, :bfgs)
        ...
        # Rate-rule state: the best Q at the close of the previous window, and how
        # many consecutive windows have improved by less than nm_rate_tol.
        window_mark   = Ref(Inf)
        window_end    = Ref(0)
        slow_runs     = Ref(0)
        ...
        #  · rate: the best Q improved by less than nm_rate_tol over each of
        #    nm_rate_runs consecutive windows of nm_rate_window evaluations. A window
        #    with exactly zero improvement resets the streak instead of counting —
        #    that is the simplex against an infeasible wall, not convergence.
        function nm_stop_cb(_state)
            ...
            if nm_rate_tol > 0 && nm_rate_window > 0 &&
               iter_count[] - window_end[] >= nm_rate_window
                gain = isfinite(window_mark[]) ? window_mark[] - best_loss[] : Inf
                window_mark[] = best_loss[]
                window_end[]  = iter_count[]
                slow_runs[]   = (gain > 0 && gain < nm_rate_tol) ? slow_runs[] + 1 : 0
                if slow_runs[] >= nm_rate_runs
                    stopped_early[] = true; stop_reason[] = :rate
                    n_c = _count_corners(best_theta[], spec)
                    @printf("  [%s EARLY STOP  iter %d]  ΔQ < %.3g per %d evals for %d windows  best=%.6e  corners=%d/%d%s\n",
                            method, iter_count[], nm_rate_tol, nm_rate_window,
                            nm_rate_runs, best_loss[], n_c, length(spec.free),
                            _corner_tags(best_theta[], spec))
                    flush(stdout)
                    return true
                end
