...
#
# Usage:  julia --project=. code/gates/check_forwarding.jl
...
# code/mcmc/ since v19.6.0. A stale path here would throw rather than pass silently,
# but the same is not true of check_dead_settings' FILES list — see the note there.
const MCMC     = read_src("code", "mcmc", "MCMC_main.jl")
const DEMC     = read_src("code", "mcmc", "demc.jl")
...
# SA has ONE call site as of v19.7.0. The `:sa` branch used to hold its own copy of the
# argument list and the union covered both; the copy had drifted (it omitted
# checkpoint_path) and was deleted rather than repaired, so `:sa` and `:sa_de`'s first stage
# now go through _sa_stage together. A single anchor is therefore correct here, and it is
# STRICTER than the union was: with two call sites a keyword present in either counted as
# forwarded, which is precisely how the checkpoint_path gap passed this gate for a version.
# DE below still unions two, because its `:de` branch does still call _run_de directly.
const SA_PASSED  = passed_at_call(SMM, "_sa_stage(sp, starts) = _run_sa(")
