...
# one-line edit here.
#
# Valid names (30 moments in MOMENT_NAMES order):
...
FIX_PARAMS = Dict{Symbol,Float64}(
    ...
    # Both exogenous separation hazards ξ_U and ξ_S are FREE (neither pinned
    # here).  Each sector's separation splits into a baseline hazard and an
    # endogenous margin: sep_rate_U = ξ_U + λ_U·G(p*_U) and
    # sep_rate_S = ξ_S + λ_S·Γ_s(p*_S).  ξ_U gives sep_rate_U a live lever when
    # the endogenous part collapses (p*_U → 0); on the skilled side δ_S (free)
    # governs the endogenous margin.  Do NOT pin :skl_delta.
)
...
const DEFAULT_PARAMS = Dict{Symbol,Float64}(
    ...
    :skl_sigw  => 0.21904,
    :skl_xi   => 0.00000,
    :skl_delta => 0.60000,          # offer/shock support ratio δ_S (<1 revives the ladder)
)
...
ACTIVE_SKIP_MOMENTS = sort(unique(vcat(SKIP_MOMENTS, _nan_targets)))
