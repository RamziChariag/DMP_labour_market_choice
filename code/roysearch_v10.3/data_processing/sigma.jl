...
#
# Combine step (Stage 8) — the full 30×30 influence-function variance–
...
#
# Reads:  moments_{window}.csv, cps_basic_clean.arrow, cps_asec_clean.arrow, jolts_clean.arrow, transitions_monthly.arrow, data/raw/j2j/*.csv, sipp_wchg_rates.csv, training_share_scale.csv
...
function compute_influence_functions_and_sigma_full()
    ...
    # SIPP within-job wage-change hazards + Kish effective N (Stage 6b). The
    # SIPP micro-data are not re-read here (only the E4 J2J and CPS raw are);
    # the point estimate and neff carried in sipp_wchg_rates.csv are enough to
    # place a delta-method variance on wchg_rate_j below.
    ...
    for (wname, wdef) in WINDOWS
        ...
        # ── G'. wchg_rate_U / wchg_rate_S variance from SIPP (external CSV) ────
        # Binomial-proportion delta method: the stored moment is the hazard
        # h = −log(1−p), so Var(h) = Var(p)·(dh/dp)² = [p(1−p)/neff]·(1−p)⁻².
        # Uses the point estimate + Kish neff from sipp_wchg_rates.csv; the
        # SIPP micro-data are not re-read. Where the window has no SIPP row the
        # diagonal is left at 0 with a warning (same pattern as the
        # ltu_share_S DURUNEMP-absent branch).
        sipp_w = isempty(sipp_wchg) ? nothing : filter(r -> r.window == wname, sipp_wchg)
        for (mname, neff_col) in ((:wchg_rate_U, :neff_U), (:wchg_rate_S, :neff_S))
            wk = idx[mname]
            ...
            if !isnothing(sipp_w) && nrow(sipp_w) > 0 && isfinite(h) && hasproperty(sipp_w, neff_col)
                ...
                p    = 1.0 - exp(-h)
                if neff > 0.0 && 0.0 < p < 1.0
                    var_h = (p * (1.0 - p) / neff) / (1.0 - p)^2
                    Sigma[wk, wk] = var_h / scales[wk]^2
                    @info "    $mname: delta-method variance (neff=$(round(neff; digits=1))) = $(round(Sigma[wk, wk]; sigdigits=4))"
                else
                    @warn "    $mname: degenerate p/neff for $wname; variance left as 0"
                end
            else
