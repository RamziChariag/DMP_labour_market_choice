...
println("="^60)
println("  RoySearch v15.2 — DE-MC standard errors")
println("="^60)
flush(stdout)
...
MCMC_RHAT_MAX    = 1.03            # 1.01 is the customary advice but is unreachable
                                   # for DE-MC at these chain counts: on an isotropic
                                   # Gaussian — the easiest target this sampler will
                                   # ever face — N=50 reaches only 1.012 after 4000
                                   # generations, so a 1.01 gate would never fire and
                                   # the rule would degrade to always spending the
                                   # full budget. 1.03 is well inside the ≤1.1 of
                                   # Gelman et al. (2004).
MCMC_ESS_MIN     = 0.0             # 0.0 ⇒ minESS(d) at the ε in min_ess (default
                                   # ε = 0.20, giving ≈540 at d = 25)
...
@printf("  gens=%d/%d %s  burn=%d  kept=%d  accept=%.3f\n",
        ...
        res.gens < res.gens_requested ? "(stopped early)" : "(budget reached)",
        res.burn, n_kept, res.accept)
@printf("  logπ(θ̂)=%.6e  max logπ=%.6e   ESS floor=%.0f (minESS, d=%d, ε=0.10)\n",
        logposterior(θ0), maximum(res.lp),
        MCMC_ESS_MIN > 0 ? MCMC_ESS_MIN : min_ess(d), d)
