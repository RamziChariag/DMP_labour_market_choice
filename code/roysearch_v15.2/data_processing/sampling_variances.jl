...
function compute_sampling_variances()
    ...
    for (wname, wdef) in WINDOWS
        ...
        # moments_{window}.csv stores the κ_w-adjusted training_share (applied in
        # Stage 7). Recover the raw level here so the closed forms are centred on
        # the raw moment; κ_ts is re-applied to the training_share variance below.
