...
const DEFAULT_PARAMS = Dict{Symbol,Float64}(
    ...
    :a_l      => 2.59266,
    :b_l      => 1.31532,
    :c        => 3.77,        # c̃ convention (old raw c = 10.58 would clamp onto the ub)
    :A        => 4.83051,
    :PU       => 6.53357,
    :gamma_PS => 9.72094,
    :bU       => 1.38551,
...
const _TEMP_BOUNDS = Dict{Tuple{Symbol,Symbol},Tuple{Float64,Float64}}(
    (:common, :c)        => (-4.0,  4.5),    # c̃ = c − A convention (solver patched):
                                             # −4 ⇒ training ~free; +4 ⇒ wall at x=1.
                                             # ub 4.5 (not 4.0) so seeds ≤ 4.32 fit.
    (:unsk,   :bU)       => (0.00,  4.5),    # viability b_U < P_U·x once P_U ~ 1-2
    (:unsk,   :bT)       => (0.00,  7.0),    # stipend can sit below unskilled output
    (:skl,    :bS)       => (0.00,  2.0),
    (:unsk,   :PU)       => (0.10,  12.0),
    ...
    (:skl,    :μ)        => (0.01,  4.5),
    (:unsk,   :λ)        => (0.0001,  2.5),    # lb keeps the shock mechanism ON;
    (:skl,    :λ)        => (0.0002, 0.5),    # 1e-4 invites the no-shock corner (see chat)
...
run_params = SMMRunParams(
    ...
    de_max_iter  = 2_000,
    de_pop_size  = 200,        # 30 converges prematurely for 18 free params
    de_f         = 0.70,
    de_cr        = 0.85,
    de_patience  = 45,            # [TEMP] was 2 — too trigger-happy
    de_avg_tol   = 1e-6,
...
const _PORT_SEEDS = NamedTuple[
    # c values are c̃ = c − A (solver patched to use cost = (1−x)·exp(c̃+A−x)).
    # Allocations and port-verified Q values are identical to the raw-c seeds.
    # 1  star seed: beats the old basin on the coarse grid (Q = 27.98)
    (PU=1.3, A=6.4451, gamma_PS=2.5, c=3.7676, bU=0.1930, bT=0.7599, bS=0.1352,
     kU=0.0377, kS=0.4175, lamU=0.3787, muU=0.3279, muS=0.2535),
    # 2  same cell, low-λ_U / calibrated-μ variant (Q = 36.24)
    (PU=1.3, A=6.4451, gamma_PS=2.5, c=3.4109, bU=0.1930, bT=0.7599, bS=0.1352,
     kU=0.0654, kS=0.4470, lamU=0.0854, muU=0.2201, muS=0.2257),
    # 3-4  PU=0.8 cells, low-λ_U (Q = 36.89, 39.98)
    (PU=0.8, A=6.9306, gamma_PS=1.5, c=3.3658, bU=0.1188, bT=0.4676, bS=0.0971,
     kU=0.0325, kS=0.2521, lamU=0.0818, muU=0.2236, muS=0.2289),
    (PU=0.8, A=6.9306, gamma_PS=2.0, c=3.5036, bU=0.1188, bT=0.4676, bS=0.1181,
     kU=0.0310, kS=0.3007, lamU=0.0818, muU=0.2515, muS=0.2314),
    # 5-7  best skilled-share cells (share ≈ 0.35, x̄ ≈ 0.63; Q = 43.61-45.23)
    (PU=0.8, A=6.9306, gamma_PS=2.0, c=2.8560, bU=0.1188, bT=0.2338, bS=0.1181,
     kU=0.0239, kS=0.3090, lamU=0.3787, muU=0.3279, muS=0.2535),
    (PU=0.8, A=6.9306, gamma_PS=2.5, c=2.7975, bU=0.1188, bT=0.2338, bS=0.1352,
     kU=0.0240, kS=0.3566, lamU=0.3787, muU=0.3279, muS=0.2535),
    (PU=0.8, A=6.9306, gamma_PS=2.5, c=3.9956, bU=0.1188, bT=0.4676, bS=0.1352,
     kU=0.0221, kS=0.3361, lamU=0.3787, muU=0.3279, muS=0.2535),
    # 8-10  diversity across (PU, γ) cells (Q = 51.46, 72.38, 96.99)
    (PU=0.8, A=6.9306, gamma_PS=1.5, c=2.8942, bU=0.2036, bT=0.4676, bS=0.0971,
     kU=0.0257, kS=0.2523, lamU=0.0818, muU=0.2723, muS=0.2284),
    (PU=1.3, A=6.4451, gamma_PS=2.0, c=3.6903, bU=0.1930, bT=0.7599, bS=0.1181,
     kU=0.0327, kS=0.3061, lamU=0.3787, muU=0.3279, muS=0.2535),
    (PU=2.0, A=6.0143, gamma_PS=4.0, c=4.3156, bU=0.2969, bT=1.1691, bS=0.1702,
     kU=0.0445, kS=0.6932, lamU=0.3787, muU=0.3279, muS=0.2535),
    # 11-14  unverified local variants of seed 1 (filtered by the driver)
    (PU=1.3, A=6.4451, gamma_PS=2.5, c=3.4549, bU=0.1930, bT=0.7599, bS=0.1352,
     kU=0.0377, kS=0.4175, lamU=0.3787, muU=0.3279, muS=0.2535),
    (PU=1.3, A=6.4451, gamma_PS=2.5, c=4.0549, bU=0.1930, bT=0.7599, bS=0.1352,
     kU=0.0377, kS=0.4175, lamU=0.1500, muU=0.2800, muS=0.2400),
    (PU=1.3, A=6.7000, gamma_PS=2.5, c=3.7700, bU=0.1930, bT=0.7599, bS=0.1352,
     kU=0.0290, kS=0.3200, lamU=0.3787, muU=0.3279, muS=0.2535),
    (PU=1.3, A=6.4451, gamma_PS=3.0, c=3.7649, bU=0.1930, bT=0.7599, bS=0.1500,
