...
function compute_equilibrium_objects(model::Model)
    ...
    PU = up.PU;  γPS = sp.gamma_PS;  bS = sp.bS * exp(cp.A);  αU = up.α_U
    ...
    for ix in 1:Nx
        ...
        solve_unskilled_surplus_on_grid!(
            Svec, pgU, wGU, exp(cp.A) * PU, xg[ix], r, ν, λU, UU[ix], uc.pstar[ix]
        )
        SU_surface[ix, :] = Svec
    end
    ...
    for ix in 1:Nx
        ...
        for jp in 1:NpU
            ...
            if ω_j > 0.0
                wU_surface[ix, jp] = βU * exp(cp.A) * PU * xg[ix] * pgU[jp] + outside
            end
        end
    end
    ...
    for ix in 1:Nx
        ...
        PS_x        = PS_of_x(xg[ix], γPS, exp(cp.A))
        flow_out    = (1.0 - βS) * bS
