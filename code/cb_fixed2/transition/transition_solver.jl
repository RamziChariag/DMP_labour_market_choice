...
#   PS(x) = γ · x^{γ−1},   γ = sp.gamma_PS,   A = exp(cp.A),
...
PS(x) is evaluated via PS_of_x(x, sp.gamma_PS, exp(cp.A)).
"""
function _backward_pass!(path::TransitionPath, model::Model, tp::TransitionParams)
    ...
    for n in (Nt - 1):-1:1
        ...
        @inbounds for ix in 1:Nx
            ...
            PS_x = PS_of_x(x_val, sp.gamma_PS, exp(cp.A))
...
function _build_result(
    ...
    for n in 1:Nt
        ...
        for ix in 1:Nx
            ...
            if eU_ix > 1e-14
                w_U_avg = exp(cp.A) * up.PU * gp.x[ix] * up.β + (1.0 - up.β) * (cp.r + cp.ν) * path.U[ix, n]
            ...
            for jp in 1:NpS
                ...
                if e_ij > 1e-14
                    pj    = sg.p[jp]
                    PS_xi = PS_of_x(gp.x[ix], sp.gamma_PS, exp(cp.A))
