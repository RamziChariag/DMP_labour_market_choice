...
if Q_best < Q_seed - 1.0
    # Back the displaced bundle up before overwriting. The suffix carries the Q it
    # held, so a directory of backups reads as the descent history of the
    # optimise-sample-reseed loop, and no two rounds collide on one filename.
    backup_jls = replace(seed_jls, r"\.jls$" => @sprintf("_backup_Q%.6f.jls", Q_seed))
    cp(seed_jls, backup_jls; force = true)

    cp_b, up_b, sp_b = unpack_θ(collect(res.theta_best), spec)
    promoted = SMMResult(collect(res.theta_best),
                         _params_to_namedtuple(cp_b, up_b, sp_b, spec),
                         Q_best, false, 0, spec)
    open(seed_jls, "w") do io
        serialize(io, (result = promoted, spec = spec))
    end
    @printf("\n  PROMOTED: Q %.6e → %.6e (ΔQ=%.2f)\n", Q_seed, Q_best, Q_seed - Q_best)
    @printf("    backed up  %s\n    overwrote  %s\n", basename(backup_jls), seed_jls)
    @printf("  Re-run smm_main.jl with INIT_MODE = :warmstart to optimise from it.\n")
else
    @printf("\n  No promotion: chain best Q %.6e is within 1 of the seed's %.6e\n",
            Q_best, Q_seed)
end
@printf("\nWrote %s\n       %s\n", out_csv, chain_jls)
flush(stdout)
