...
run_params = SMMRunParams(
    ...
    sa_cooling_exp     = 1.8,     # exponent in same schedule (higher = faster cool)
    ...
    sa_max_reheats     = _env_int("ROYSEARCH_SA_MAX_REHEATS", 20),  # cap per run, 0 = unlimited
    ...
    # Only f_reltol is scale-free, so it is the intended stopping rule and the other
    # two are backstops.  The floor on all of them is the solver: with
    # tol_global = 1e-4 each moment carries ~1e-4 relative error, which at the
    # variance weights propagates to a Q uncertainty of roughly ±13 on Q ≈ 5e3, i.e.
    # 3e-3 in relative terms.  A tolerance below that has the simplex chasing solver
    # noise rather than the objective, so f_tol is set an order ABOVE the noise and
    # tightened only together with tol_global.
    nm_max_iter  = _env_int("ROYSEARCH_NM_MAX_ITER", 7_000),  # max NM iterations
    nm_f_tol     = _env_f64("ROYSEARCH_NM_F_TOL", 1e-2),   # f_reltol: |f_hi−f_lo| ≤ tol·|f|
                                  # 1e-2 ≈ 3× the 3e-3 noise floor at tol_global=1e-4.
                                  # Drop to 1e-3 with tol_global=1e-5, 1e-4 with 1e-6.
    nm_x_tol     = _env_f64("ROYSEARCH_NM_X_TOL", 1e-6),   # x_abstol: simplex diameter in t.
    ...
    nm_g_tol     = _env_f64("ROYSEARCH_NM_G_TOL", 0.0),    # g_abstol: 0 disables.
                                  # Nelder–Mead has no gradient; Optim maps this onto
                                  # the simplex spread as an ABSOLUTE test, so any
                                  # positive value is a hidden second f-tolerance on a
                                  # Q whose scale changes with the weighting.  The old
                                  # 1e-5 was 2e-9 in relative terms — unreachable, and
                                  # the reason converged came back false.
...
# Diagnostic escape hatch: serialise the spec this run assembled and stop before the
# optimiser.  A warm start that scores Inf at a seed which reproduces under its own
# saved spec differs in the spec, not in θ — and comparing the two requires the real
# assembled object rather than a reconstruction of it.  Costs nothing when unset.
if _env_bool("ROYSEARCH_DUMP_SPEC", false)
    mkpath(SMM_OUT_DIR)
    dump_path = joinpath(SMM_OUT_DIR, "spec_dump_$(WINDOW)$(W_SUFFIX).jls")
    serialize(dump_path, (spec = spec, sim = sim_smm, window = WINDOW))
    @printf("\nROYSEARCH_DUMP_SPEC — wrote %s and stopped before the optimiser.\n",
            dump_path)
    exit(0)
end
