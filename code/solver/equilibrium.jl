...
function compute_equilibrium_objects(model::Model)
    ...
    βU = up.β;  λU = up.λ
    βS = sp.β;  λS = sp.λ;  σS = sp.σ * cp.A;  ξS = sp.ξ;  ρ_NILF = sp.ρ_NILF
    ...
    # constant hazard δ_S(x) = κ_S·(1 − Γ(p*_S(x))) + ρ_NILF (job-finding plus
    # the gross U→NILF competing risk). Within a type the spell is exponential;
    ...
    δS_unemp    = κS .* (1.0 .- ΓpstarS) .+ ρ_NILF
