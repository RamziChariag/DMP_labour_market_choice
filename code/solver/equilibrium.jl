...
function compute_equilibrium_objects(model::Model)
    ...
    c_of_a  = a -> training_cost(a, cp.c, cp.A)   # pecuniary cost: carries exp(A)
    ...
    for i in 1:Nx
        Svec = zeros(NpU)
        solve_unskilled_surplus_on_grid!(Svec, pgU, wGU, PU * xg[i], r, ν, λU, Usearch[i], clamp01(uc.pstar[i]))
        SU_surface[i, :] = Svec
    end
    ...
    # Unskilled: employed mass is 2D; wage reads aU only, so aggregate the
    # p-shape row mass eU_surface[i,·] (already the aU-row mass) directly.
    ...
    # ── Transition rates ──────────────────────────────────────────────────
    ΓpstarS = [pre.Γvals[pcut_index(pg, clamp01(pstar_S[j]))] for j in 1:Nx]

    # Skilled job-finding, averaged over the skilled-unemployed composition:
    # d=0 accept S-offers at κ_S(1−Γ(p*_S)); d=1 accept U-offers at f_U.
    ...
    # Unskilled separation hazard δ_U(aU) = λ_U p*(aU)^{α_U}, employed-weighted.
    δU_by_a    = λU .* clamp01.(pstar_U) .^ αU
    sep_rate_U = dot(δU_by_a, eU_rowU) / max(agg_eU, 1e-14)

    # Skilled separation hazard δ_S(aS) = ξ_S + λ_S Γ(p*_S), employed-weighted.
    δS_by_a    = ξS .+ λS .* ΓpstarS
    sep_rate_S = dot(δS_by_a, eS_totS) / max(agg_eS, 1e-14)

    # Skilled E-to-E rate: OJS-searchers (p < p^oj) poach at κ_S(1−Γ(p^oj)).
    ee_mass = 0.0;  ee_flow = 0.0
    for j in 1:Nx
        poj_j = clamp01(poj[j]);  Gamma_poj = pre.Γvals[pcut_index(pg, poj_j)]
        for jp in 1:NpS
            e_ij = eS_pS[j, jp] * wpS[jp]
            pg[jp] < poj_j && (ee_flow += e_ij * κS * (1.0 - Gamma_poj))
            ee_mass += e_ij
        end
    end
    ...
    return (
        ...
        ee_rate_S = ee_rate_S, ltu_share_S = ltu_share_S,
