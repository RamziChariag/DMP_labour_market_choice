...
function smm_objective(
    ...
    # Guard: reject solutions where θ is suspiciously close to the initial
    # seed (0.5).  A legitimate equilibrium θ is determined by the free-entry
    # condition and will generically differ from the seed.  If the update
    # function returned the seed unchanged every iteration (dead-market
    # fallback that was only partially fixed), the model is not identified.
    _θ_seed = 0.5
    if abs(model.unsk_cache.θ - _θ_seed) < 1e-10 ||
       abs(model.skl_cache.θ  - _θ_seed) < 1e-10
        return Inf
    end
