...
# treated as independent, as in LMR). It writes a parameter table keyed
# to the point estimates by (block, name):
#
...
EST_W_COND_TARGET = 2.0           # weighting the seed bundle was estimated under
                                  #   (equal/relative weights = 2.0) → which bundle to load
SKIP_MOMENTS      = Symbol[]      # moments given zero weight; must equal the set
                                  #   used when the bundle was built
...
_w_suffix(ct::Float64) = ct == 0.0 ? "_diagonalW" :
                         ct == 2.0 ? "_equalW" :
                         error("EST_W_COND_TARGET must be 0.0 (diagonal-σ) or 2.0 (equal weights); got $ct.")
...
# Effective skip set for THIS bundle: the moments the estimation held out
# (weight == 0 in the saved spec), which already include any auto-skipped NaN
# targets (e.g. wchg_rate_j when the window had no SIPP). Deriving it from the
# bundle keeps the diagonal W sized to the same active set the spec rebuild
# uses, regardless of the CONFIG SKIP_MOMENTS above.
BUNDLE_SKIP = Symbol[k for k in keys(spec0.moments) if spec0.moments[k].weight <= 0.0]
if Set(BUNDLE_SKIP) != Set(SKIP_MOMENTS)
    @printf("  Note: using bundle's held-out set (%s) rather than CONFIG SKIP_MOMENTS.\n",
            isempty(BUNDLE_SKIP) ? "(none)" : join(string.(sort(BUNDLE_SKIP)), ", "))
    flush(stdout)
end

# ========================================================================
# 2. Diagonal weight  W = Diagonal(1/σ̂²_samp)  (LMR's 1/SD²)
# ========================================================================
W_diag  = load_weight_matrix(; window=WINDOW, derived_dir=derived_dir,
                               cond_target=0.0, skip_moments=BUNDLE_SKIP)
# Display-only Q divisor: the reported Q is the raw g'Wg magnitude (q_scale = 1.0,
# constant in θ, so the quasi-posterior −0.5·Q·q_scale is unchanged).
Q_SCALE = 1.0
...
let lp0 = logposterior(θ0)
    ...
    isfinite(lp0) || error("Seed θ̂ is infeasible under the diagonal-W objective — " *
                           "check sampling_var_$(WINDOW).csv exists and SKIP_MOMENTS matches.")
end
