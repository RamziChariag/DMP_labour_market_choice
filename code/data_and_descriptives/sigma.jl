...
# matrix. The training_share row/col of the SAVED Σ̂ carries the enrolment-target
...
#
# Reads:  moments_{window}.csv, cps_basic_clean.arrow, cps_asec_clean.arrow, jolts_clean.arrow, transitions_monthly.arrow, sipp_wchg_rates.csv, sipp_ee_rates.csv
...
function compute_influence_functions_and_sigma_full()
    ...
    for (wname, wdef) in WINDOWS
        ...
        # training_share is read from the enrolment target
        # (nsc.jl::compute_training_share_target), not built from the CPS closed
        # forms here, so there is no level scale to undo.
        # NOTE: this file is not included by data_processing_main.jl.
        ...
        for gk in groupby(asec_w, :YEAR)
            ...
            if nrow(unskilled) > 0
                ...
                for (sym, fn) in ((:mean_wage_U, wmean), (:emp_var_U, wvar),
                                   (:emp_cm3_U, wcm3),
                                   (:p10_wage_U, wpercentile10),
                                   (:p25_wage_U, wpercentile25),
                                   (:p50_wage_U, wmedian),
                                   (:p75_wage_U, wpercentile75),
                                   (:p90_wage_U, wpercentile90))
                    val  = fn(wu, wt)
            ...
            if nrow(skilled) > 0
                ...
                for (sym, fn) in ((:mean_wage_S, wmean), (:emp_var_S, wvar),
                                   (:emp_cm3_S, wcm3),
                                   (:p10_wage_S, wpercentile10),
                                   (:p25_wage_S, wpercentile25),
                                   (:p50_wage_S, wmedian),
                                   (:p75_wage_S, wpercentile75),
                                   (:p90_wage_S, wpercentile90))
                    val  = fn(ws, wt)
                    mbar = get(moment_vals, sym, NaN)
                    isfinite(val) && isfinite(mbar) && (psi[idx[sym]] = val - mbar)
                end
        ...
        # comes from the enrolment target together with its across-Fall-year variance.
        ...
        for (sym, x, w, p) in ((:p10_wage_U, xu, wu, 0.10), (:p10_wage_S, xs, ws, 0.10),
                               (:p25_wage_U, xu, wu, 0.25), (:p25_wage_S, xs, ws, 0.25),
                               (:p50_wage_U, xu, wu, 0.50), (:p50_wage_S, xs, ws, 0.50),
                               (:p75_wage_U, xu, wu, 0.75), (:p75_wage_S, xs, ws, 0.75),
                               (:p90_wage_U, xu, wu, 0.90), (:p90_wage_S, xs, ws, 0.90))
        ...
        # ── I. Save ───────────────────────────────────────────────────────────
        # training_share carries no level scale: it is read from the enrolment
        # target (nsc.jl), so the saved Σ̂ needs no row/column adjustment. Its
        # diagonal entry is overwritten above with the across-year variance; the
        # off-diagonals are left as the influence-function construction produced
        # them, which is the honest statement that this file cannot estimate a
        # covariance between an administrative enrolment series and a CPS moment.
