...
function _sa_loop(
    ...
    for t in 1:max_iter
        ...
        t_local  += 1
        # Normalised logarithmic cooling.  The bare form T_reheat/log(1+rate·t)^exp
        # divides by a number BELOW ONE while 1+rate·t < e, so it *heats* over the
        # first (e−1)/rate steps — at rate = 0.1, exp = 4 the first step overshoots
        # T_reheat by four orders of magnitude.  Dividing by log(1+rate)^exp pins
        # T(1) = T_reheat exactly for every (rate, exp), so the two knobs control
        # only the decay profile and never the starting level.
        T_current = T_reheat * (_COOL_NUM / log1p(cooling_rate * t_local))^cooling_exp
...
function Optim.simplexer(S::FeasibleSimplexer, initial_x::Tx) where {Tx}
    ...
    nfeas   = Threads.Atomic{Int}(0)
    # Bisection per coordinate is embarrassingly parallel and costs d·nbisect solves
    # once, against the thousands the default wastes on infeasible vertices.
    Threads.@threads for j in 1:d
        lo, hi = 0.0, S.step
        for _ in 1:12
            mid = 0.5 * (lo + hi)
            t   = copy(initial_x); t[j] += mid
            isfinite(smm_objective(t, S.spec)) ? (lo = mid) : (hi = mid)
        end
        # A coordinate with no feasible step at all keeps the seed value: the simplex
        # is then degenerate in that direction, which is the honest representation of a
        # parameter the objective cannot move.
        simplex[j+1][j] += lo
        lo > 0 && Threads.atomic_add!(nfeas, 1)
    end
    @printf("  [NM simplex]  %d/%d coordinates admit a feasible step (cap %.3g)\n",
            nfeas[], d, S.step)
    flush(stdout)
    simplex
end
...
function run_smm(
    ...
    # Initial-simplex step cap in transformed units. Optim's default places every vertex
    # far outside the feasible region here (median 296x the half-width at base_fc), so
    # the first thousands of evaluations score Inf and only shrink the simplex. A
    # positive value bisects each coordinate for the largest feasible step below this
    # cap; 0 restores Optim's AffineSimplexer. Raising the cap cannot make a vertex
    # infeasible — bisection always returns a feasible step — so it only limits how far
    # the WIDEST coordinates step first. At the base_fc optimum the measured half-widths
    # top out at 1.1e-1, so 0.2 binds on none of the 25 and the geometry sets every
    # step. Lower it only to deliberately short-step the widest coordinates.
    ...
    elseif method in (:neldermead, :lbfgs, :bfgs)
        ...
        function nm_stop_cb(_state)
            ...
            if nm_rate_tol > 0
                # Continuous accounting, no windows. An evaluation that moved the
                # best Q counts toward the slow budget; one that did not PAUSES it —
                # neither advancing nor resetting — because a flat stretch is the
                # simplex against an infeasible wall, which in this many dimensions
                # can persist for thousands of evaluations and then recover.
                moved = best_loss[] < last_seen_Q[]
                # rate_mark_it advances on EVERY call, so evaluations spent paused are
                # discarded rather than deferred: without this the next productive
                # evaluation would add the whole paused gap and the pause would count
                # after all.
                gap            = iter_count[] - rate_mark_it[]
                rate_mark_it[] = iter_count[]
                if moved
                    slow_evals[] += gap
                    last_seen_Q[] = best_loss[]
                    if !isfinite(rate_mark_Q[])
                        rate_mark_Q[] = best_loss[]
                        slow_evals[]  = 0
                    else
                        # Rate since the mark, expressed per 100 evaluations so the
                        # tolerance reads in the same units whatever the span.
                        rate = (rate_mark_Q[] - best_loss[]) / max(slow_evals[], 1) * 100.0
                        if rate >= nm_rate_tol
                            rate_mark_Q[] = best_loss[]
                            slow_evals[]  = 0
                        elseif slow_evals[] >= nm_rate_span
                            stopped_early[] = true; stop_reason[] = :rate
                            n_c = _count_corners(best_theta[], spec)
                            @printf("  [%s EARLY STOP  iter %d]  ΔQ < %.3g per 100 evals over %d evals  best=%.6e  corners=%d/%d%s\n",
                                    method, iter_count[], nm_rate_tol, slow_evals[],
                                    best_loss[], n_c, length(spec.free),
                                    _corner_tags(best_theta[], spec))
                            flush(stdout)
                            return true
