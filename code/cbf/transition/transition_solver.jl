...
#   PS(x) = P_S · x,   P_S = sp.PS,   A = exp(cp.A),
# via the PS_of_x(x, P_S, A) helper defined in grids.jl.
# NOTE: cp.A is the LOG scale (see params.jl); the effective multiplier
# everywhere is exp(cp.A), matching the stationary solver.
#
# Public API
...
PS(x) is evaluated via PS_of_x(x, sp.PS, exp(cp.A)).
"""
function _backward_pass!(path::TransitionPath, model::Model, tp::TransitionParams)
    ...
    for n in (Nt - 1):-1:1
        ...
        # Unskilled-side link variables from date n+1 (backward timing),
        # mirroring solver.jl step B:
        #   E_U(x,1) = U_U(x) + β_U · S_U(x,1),  S_U(x,1) = J_frontier/(1−β_U).
        # These feed the U^(1) branch of the skilled discrete choice d(x,t).
        EU1_next = [path.U[ix, n + 1] +
                    up.β * path.Jfrontier[ix, n + 1] / max(1.0 - up.β, 1e-14)
                    for ix in 1:Nx]

        # Run the skilled inner loop to convergence at this θS
        # (updates sc.U, surfaces, and the cross-market policy sc.d)
        skilled_inner_loop!(model; mS_in = mS_n, fU = fU, EU1 = EU1_next)
        ...
        @inbounds for ix in 1:Nx
            ...
            # PS(x) for this worker type — fct_ps variant (effective scale exp(A))
            PS_x = PS_of_x(x_val, sp.PS, exp(cp.A), sp.γ_S)   # GAMMA_S
            ...
            @inbounds for j in 1:NpS
                pj = sg.p[j]
                # Match flow output:  A · P_S · g(x) · p   (EXP-MAP; PS_x = A·P_S·g(x))
                flow = PS_x * pj
                raw_S0 = (flow - (cp.r + cp.ν) * U_x + sp.λ * I_val) / base
                tail_mass_j = pre.tail_weights[j]
                tail_Emax_j = tailE[max(j, j0_soft)]
                raw_S1 = (flow - (cp.r + cp.ν) * U_x - sp.σ * exp(cp.A) + sp.λ * I_val +
        ...
        # τ gate: carry the date-n cross-market policy d(x, t_n) (just
        # computed by the skilled inner loop) into the unskilled training
        # decision — same equilibrium refinement as the stationary solver:
        # a worker with d = 1 does not initiate training.
        copyto!(uc.d_carry, sc.d)
...
function _update_tightness!(path::TransitionPath, model::Model, tp::TransitionParams)
    ...
    for n in 1:Nt
        ...
        if J_u_sum > 1e-12 && u_U_sum > 1e-12
            q_U     = up.k * mean_output_U(model) * u_U_sum / J_u_sum
        ...
        if num_S > 1e-12 && den_S > 1e-12
            Jbar_S  = num_S / den_S
            q_S     = sp.k * mean_output_S(model) / Jbar_S
...
function _build_result(
    ...
    for n in 1:Nt
        ...
        for ix in 1:Nx
            ...
            # Unskilled wages: w_U(x,p) = PU·g(x)·p·β + (1-β)·(r+ν)·U(x)   (EXP-MAP)
            # Average over employed — approximate using frontier (p=1)
            if eU_ix > 1e-14
                w_U_avg = exp(cp.A) * up.PU * exp(up.γ_U * gp.x[ix]) * up.β + (1.0 - up.β) * (cp.r + cp.ν) * path.U[ix, n]   # GAMMA_U
            ...
            for jp in 1:NpS
                ...
                if e_ij > 1e-14
                    pj    = sg.p[jp]
                    PS_xi = PS_of_x(gp.x[ix], sp.PS, exp(cp.A), sp.γ_S)   # GAMMA_S: A·P_S·exp(γ_S·x)
                    w_S_ij = PS_xi * pj * sp.β +
