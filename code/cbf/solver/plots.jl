...
Skilled employment density heatmap with P_S(x) = P_S·x on the
horizontal axis and match quality p on the vertical axis.

The GL quadrature x-grid is non-uniformly spaced, and the linear map
x → P_S·x preserves that non-uniform spacing.  Passing a
...
Fix: compute PSg = P_S·xg on the GL grid (monotone since P_S > 0),
build a UNIFORM PS grid that spans [PSg[1], P_S] with the same number
...
function fig_skilled_employment_by_PS(obj, PS::Float64;
    ...
    PSg = [PS_of_x(x, PS) for x in xg]   # matches solver convention

    # ── Uniform PS grid: same length as xg, spans [PSg[1], P_S] ─────────
    # PSg[1] = P_S·xg[1] ≈ 0;  PSg[end] ≈ P_S (PS at x≈1)
    PS_lo      = PSg[1]
    PS_hi      = PS                        # theoretical max = PS(1) = P_S
...
function make_all_plots(obj; output_dir::String = "output/plots", PS::Union{Float64,Nothing} = nothing)
    mkpath(output_dir)
    _set_theme!()
