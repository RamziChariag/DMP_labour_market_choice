...
function compute_equilibrium_objects(model::Model)
    ...
    PU = up.PU;  γ_U = up.γ_U;  P_S = sp.PS;  γ_S = sp.γ_S;  bS = sp.bS * exp(cp.A);  αU = up.α_U   # GAMMA_S / GAMMA_U

    c_of_x = x -> exp(cp.A) * training_cost(x, cp.c)   # dollar cost = exp(A)·c(x)
    ...
    # Diagnostic: types where training would pay (net_T ≥ U^search) but is
    # blocked by the cross-market gate (d = 1 ⇒ τ forced to 0).  A nonzero
    # count means the parameter point lies in the excluded train-then-
    # search-unskilled region; in valid equilibria this is identically 0.
    gate_bind   = [(net_T[ix] >= Usearch[ix]) && (d_vec[ix] > 0.5) for ix in 1:Nx]
    n_gate_bind = count(gate_bind)
    ...
    for ix in 1:Nx
        ...
        solve_unskilled_surplus_on_grid!(
            Svec, pgU, wGU, exp(cp.A) * PU, xg[ix], r, ν, λU, UU[ix], uc.pstar[ix], γ_U   # GAMMA_U
        )
        SU_surface[ix, :] = Svec
    end
    ...
    for ix in 1:Nx
        ...
        for jp in 1:NpU
            ...
            if ω_j > 0.0
                wU_surface[ix, jp] = βU * exp(cp.A) * PU * exp(γ_U * xg[ix]) * pgU[jp] + outside  # GAMMA_U
            end
        end
    end
    ...
    for ix in 1:Nx
        ...
        PS_x        = PS_of_x(xg[ix], P_S, exp(cp.A), γ_S)
        flow_out    = (1.0 - βS) * bS
        ladder_term = βS * (1.0 - βS) * κS * I_full[ix]
        for jp in 1:NpS
            ω_j = _soft_weight(pg[jp], pst, pg, jp, NpS)
            if ω_j > 0.0
                wS0_surface[ix, jp] = βS * PS_x * pg[jp] + flow_out + ladder_term  # EXP-MAP: PS_x = A·P_S·g(x)
            end
        end
    end
    ...
    for ix in 1:Nx
        ...
        for jp in 1:NpS
            ...
            if ω_j > 0.0
                ...
                wS1_surface[ix, jp] =
                    βS * PS_x * pg[jp] +                      # EXP-MAP: PS_x = A·P_S·g(x)
                    flow_out +
                    βS * (1.0 - βS) * κS * I_low
            end
        end
    ...
    # Skilled separation hazard into unemployment: exogenous ξ_S plus
    # endogenous λ_S · Γ(p*_S(x)).  ξ_S = 0 recovers the purely endogenous case.
    δS_by_x    = ξS .+ λS .* ΓpstarS
    ...
    # ── Skilled long-term-unemployment share ──────────────────────────────
    # In stationary equilibrium a skilled-unemployed type-x worker exits at the
    # constant hazard δ_S(x) = κ_S·(1 − Γ(p*_S(x))) + ν (job-finding plus
    # demographic exit ν — the model's only route out of the LF). Within a type the spell is exponential;
    # the unemployed pool is the u_S-weighted mixture, so the share still
    # unemployed after a* = 27·12/52 ≈ 6.23 months is the mixture survivor
    # ∫ u_S(x) e^{−δ_S(x) a*} dx / ∫ u_S(x) dx. The long tail is generated by
    # heterogeneity in δ_S(x), identifying the dispersion of skilled
    # job-finding (skilled-only: the unskilled side accepts homogeneously).
    a_star      = 27.0 * 12.0 / 52.0
    δS_unemp    = κS .* (1.0 .- ΓpstarS) .+ ν
    ltu_share_S = uS_mass > 1e-14 ?
                  dot(uS .* wx, exp.(-δS_unemp .* a_star)) / dot(uS, wx) : 0.0
    ...
    return (
        ...
        gate_bind   = gate_bind, n_gate_bind = n_gate_bind,
        ...
        thetaU  = begin
            ...
            (_Jbar > 1e-14 && _Utot > 1e-14) ?
                max(theta_from_q(up.k * mean_output_U(model) * _Utot / _Jbar, up.μ, up.η), 1e-14) : 1e-14
        end,
        thetaS  = begin
            _JS = compute_Jbar_skilled(model)
            _JS > 1e-12 ?
                max(theta_from_q(sp.k * mean_output_S(model) / _JS, sp.μ, sp.η), 1e-14) : 1e-14
        end,
        ...
        # Skilled long-term-unemployment share (survival past a* ≈ 6.23 months)
