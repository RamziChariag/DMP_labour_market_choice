...
function skilled_inner_loop!(
    ...
    r  = cp.r;   ν  = cp.ν
    P_S = sp.PS;  γ_S = sp.γ_S;  bS = sp.bS * exp(cp.A)   # GAMMA_S
    ...
    for it in 1:sim.maxit_inner
        ...
        @threads for ix in 1:Nx
            ...
            @inbounds begin
                x     = gp.x[ix]
                PS_x  = PS_of_x(x, P_S, exp(cp.A), γ_S)
                ...
                for j in 1:Np
                    ...
                    # No-search surplus  (eq 28).   EXP-MAP: PS_x = A·P_S·g(x)
                    S0 = smooth_pos((PS_x * pj - (r + ν) * U_new + λ * I) / base)

                    # OJS surplus  (eq 29).
                    tail_mass_j = pre.tail_weights[j]
                    tail_Emax_j = tailE[max(j, j0_soft)]
                    S1 = smooth_pos((PS_x * pj - (r + ν) * U_new - σ + λ * I +
    ...
    for it in 1:sim.maxit_outer
        ...
        @threads for ix in 1:Nx
            @inbounds begin
                ...
                for j in 1:Np
                    pj = sg.p[j]
                    raw_S0 = (PS_x * pj - (r + ν) * U_x + λ * I) / base   # EXP-MAP: PS_x = A·P_S·g(x)

                    tail_mass_j = pre.tail_weights[j]
                    tail_Emax_j = tailE[max(j, j0_soft)]
                    denom_S1 = base + f * tail_mass_j
                    raw_S1 = (PS_x * pj - (r + ν) * U_x - σ + λ * I +
