...
function compute_influence_functions_and_sigma_full()
    ...
    for (wname, wdef) in WINDOWS
        ...
        # training_share is read from NSC (nsc.jl::compute_training_share_target),
        # not built from the CPS closed forms here, so there is no level scale to
        # undo. NOTE: this file is not included by data_processing_main.jl.
        ...
        # comes from NSC together with its across-Fall-year variance.
        ...
        # ── I. Save ───────────────────────────────────────────────────────────
        # training_share carries no level scale: it is read from NSC directly
        # (nsc.jl), so the saved Σ̂ needs no row/column adjustment. Its diagonal
        # entry is overwritten above with the NSC across-year variance; the
        ...
        # covariance between an NSC administrative series and a CPS moment.
