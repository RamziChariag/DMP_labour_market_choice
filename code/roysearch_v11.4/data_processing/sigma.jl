...
function compute_influence_functions_and_sigma_full()
    ...
    for (wname, wdef) in WINDOWS
        ...
        # hazard moments → binomial-proportion delta method. Each entry names the
        # moment, its source table, and the neff column within that table.
        hazard_specs = (
            (:wchg_rate_U, sipp_w,  :neff_U),
            (:wchg_rate_S, sipp_w,  :neff_S),
            (:ee_rate_S,   sipp_er, :neff_ee),
        )
        for (mname, src, neff_col) in hazard_specs
            wk = idx[mname]
            h  = get(moment_vals, mname, NaN)
            if !isnothing(src) && nrow(src) > 0 && isfinite(h) && hasproperty(src, neff_col)
                neff = Float64(src[1, neff_col])
                p    = 1.0 - exp(-h)
        ...
        if !isnothing(sipp_er) && nrow(sipp_er) > 0 &&
           hasproperty(sipp_er, :ee_step_sd) && hasproperty(sipp_er, :neff_step)
            ...
            if isfinite(sd) && neff > 0.0
                Sigma[step_k, step_k] = (sd^2 / neff) / scales[step_k]^2
