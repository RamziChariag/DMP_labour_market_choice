...
function make_moments()
    ...
    # borrowing). A window with no row ⇒ wchg_rate_j left NaN, which auto-holds the
    # moment out of the SMM objective for that window (the FC windows have no SIPP
    # until the classic 2004/2008 panels are added — see data-and-moments §sipp).
    ...
    # (mean EE-move wage step). ee_rate_S is now sourced from SIPP; the J2J value
    # loaded above is retained only for the printed validation comparison below.
    ...
    for (wname, wdef) in WINDOWS
        ...
        # C. Skilled EE moments. ee_rate_S is sourced from SIPP (own-panel
        #    employer-to-employer moves; Stage 6b) and ee_step_S is the mean
        #    EE-move wage step. The J2J EE rate (Stage 5) is NO LONGER the
        #    target — it is kept only for the validation comparison printed
        #    here (SIPP vs J2J per window) for the paper's data panel. Switch
        #    the target back by reading j2j_w.ee_rate_S[1] into moments[:ee_rate_S].
        #    ee_rate_S arrives as a monthly hazard −log(1−p) from sipp.jl (like
        #    wchg), so it is EXCLUDED from the frequency-consistency transform in
        #    Section F below — re-converting would apply −log(1−p) twice.
        ...
        moments[:ee_rate_S] = ee_sipp
        moments[:ee_step_S] = (!isnothing(sipp_e) && nrow(sipp_e) > 0) ? sipp_e.ee_step_S[1] : NaN
        @printf("    ee_rate_S source comparison  %-13s SIPP=%s  J2J=%s  (target ← SIPP)\n",
                string(wname),
        ...
        #     The production target is the EARNINGS-based wchg (columns
        #     wchg_rate_U/S). sipp_wchg_rates.csv also carries a BBG rate-based
        #     diagnostic (wchg_rate_U_rate/S_rate, hourly subsample) the user is
        #     inspecting; to adopt it as the target, read those two columns here
        #     instead — a one-line change, no MOMENT_NAMES / target / Σ̂ edit.
        ...
        # ee_rate_S, wchg_rate_U and wchg_rate_S are DELIBERATELY absent here:
        # sipp.jl already returns the −log(1−p) hazard for all three SIPP
        # transition moments, so re-converting would apply the transform twice.
        # (ee_rate_S was J2J-sourced as a probability in earlier versions and
        # WAS converted here; now that it is SIPP-sourced it must NOT be.)
