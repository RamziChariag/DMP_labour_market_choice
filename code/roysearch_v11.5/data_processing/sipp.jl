...
# One file's private accumulator: window-keyed wchg + EE counts, plus the
# calendar-2013 instrument tallies for the overlap diagnostic — earnings-based
# (d13_classic/redesign) and BBG rate-based (d13_classic_rate/redesign_rate, the
# hourly weekly-equivalent construction). `wchg_rate`, `hshare` and `rate_drop`
# carry the BBG rate-based diagnostic (rate-based wage-change counts, hourly-
# worker share, and the count of hourly person-month slots dropped for a missing
# rate/usual-hours reading); none of these feed a targeted moment. No two files
# ever share one of these during the parallel phase.
struct SippAcc
    ...
    d13_classic      :: WchgCounts          # 2013 overlap, earnings-based
    d13_redesign     :: WchgCounts
    d13_classic_rate :: WchgCounts          # 2013 overlap, BBG rate-based (hourly weekly-equivalent)
    d13_redesign_rate:: WchgCounts
end
...
function _sipp_merge!(master::SippAcc, f::SippAcc)
    ...
    _wchg_merge!(master.d13_classic_rate,  f.d13_classic_rate)
    _wchg_merge!(master.d13_redesign_rate, f.d13_redesign_rate)
end
...
function _process_person!(acc::SippAcc, recs::Vector{PMRecord}, instrument::Symbol)::Int
    ...
    @inbounds for i in 1:length(recs)-1
        ...
        #     diagnostic). The 2013 overlap tally is kept on both scales too, so
        #     classic↔redesign can be compared earnings-vs-earnings AND on the
        #     rate-based construction (per instrument).
        ...
        d13r = yr13 ? (instrument == :classic ? acc.d13_classic_rate : acc.d13_redesign_rate) : nothing
        for s1 in r1.jobs
            ...
            # quantity is already weekly). The same construction feeds the window
            # production diagnostic (wcr) and the 2013 overlap (d13r). Non-hourly
            # / rate-missing pairs simply do not enter it; the drop count is
            # tallied at parse time.
            if s1.hourly && s2.hourly && isfinite(s1.rate_wk) && isfinite(s2.rate_wk)
                r1w = deflate_wage(s1.rate_wk, cpi1, SIPP_CPIU_BASE)
                r2w = deflate_wage(s2.rate_wk, cpi2, SIPP_CPIU_BASE)
                isnothing(wcr)  || _wchg_record_pair!(wcr,  skilled, wgt, r1w, r2w)
                isnothing(d13r) || _wchg_record_pair!(d13r, skilled, wgt, r1w, r2w)
            end
        end
...
# within-job wage-change hazard on 2013 SEPARATELY through each instrument
# isolates the classic-vs-redesign measurement difference from any true calendar
# variation. It is computed on BOTH constructions — earnings-based (the
# production wage-change variable) and BBG rate-based (hourly weekly-equivalent
# subsample) — so the pure survey-instrument effect can be read on a single
# variable and the classic-rate ↔ redesign-earnings bridge can be tested. It
# touches no moment, target, Σ̂ or the objective — 2013 is never added to a window.
#
# Reading the table: if a ratio is roughly constant across ε, the corresponding
# difference is a multiplicative rescale (a single conversion factor is
# defensible). If it drifts with ε, the two sides differ in the SHAPE of the
# wage-change distribution and no single ratio can reconcile them.
#   • ratio_ee = classic_earn / redesign_earn — the earnings-based instrument
#     control (both sides on the production construction).
#   • ratio_rr = classic_rate / redesign_rate — same variable on both sides
#     (BBG hourly weekly-equivalent), isolating the pure survey-instrument effect.
#   • ratio_re = classic_rate / redesign_earn — the BRIDGE: does the classic
#     hourly-rate hazard track the redesign-earnings production moment with a
#     constant factor? A stable ratio_re would license using classic-rate as a
#     stand-in for the redesign-earnings target up to one multiplicative constant.
# 2013 hourly neff is small — printed per cell so the user can judge reliability.
# Either 2013 source absent ⇒ skip.
function print_sipp_2013_overlap(master::SippAcc)
    c  = master.d13_classic;       r  = master.d13_redesign
    cr = master.d13_classic_rate;  rr = master.d13_redesign_rate
    ...
    haz(cnt, k, mkt) = mkt == :U ? _sipp_hazard(cnt.chg_U[k], cnt.den_U[k]) :
                                    _sipp_hazard(cnt.chg_S[k], cnt.den_S[k])
    nff(cnt, k, mkt) = mkt == :U ? _sipp_neff(cnt.den_U[k], cnt.den2_U[]) :
                                    _sipp_neff(cnt.den_S[k], cnt.den2_S[])
    ratio(a, b)      = (isfinite(a) && isfinite(b) && b > 0.0) ? a / b : NaN

    _one(mkt) = begin
        println("\n  ── 2013 classic-vs-redesign overlap, market $mkt ",
                "(within-job wage-change hazard on calendar 2013 only) ──")
        @printf("  %-5s %11s %11s %11s %11s %9s %9s %9s %8s %8s\n",
                "eps", "clsc_earn", "clsc_rate", "rdsn_earn", "rdsn_rate",
                "ratio_ee", "ratio_rr", "ratio_re", "neff_ce", "neff_cr")
        for k in eachindex(SIPP_WCHG_EPS_GRID)
            ce = haz(c, k, mkt);  crt = haz(cr, k, mkt)
            re = haz(r, k, mkt);  rrt = haz(rr, k, mkt)
            @printf("  %-5g %11.5f %11.5f %11.5f %11.5f %9.3f %9.3f %9.3f %8.1f %8.1f\n",
                    SIPP_WCHG_EPS_GRID[k], ce, crt, re, rrt,
                    ratio(ce, re), ratio(crt, rrt), ratio(crt, re),
                    nff(c, k, mkt), nff(cr, k, mkt))
        end
    end
    println("\n  ══ 2013 classic-vs-redesign overlap (earnings- AND rate-based; print-only, no moment) ══")
    _one(:U)
    _one(:S)
    println("  Guide: ratio ≈ constant across ε ⇒ multiplicative reconciliation defensible;")
    println("         ratio drifting with ε ⇒ shape difference, no single factor.")
    println("  ratio_ee = classic_earn/redesign_earn (control); ratio_rr = classic_rate/redesign_rate")
    println("  (pure instrument effect); ratio_re = classic_rate/redesign_earn (the bridge to the target).")
    println()
    return nothing
end
