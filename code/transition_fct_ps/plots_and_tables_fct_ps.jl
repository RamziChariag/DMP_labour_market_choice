...
#
# Transition dynamics panels live in transition_panel_fct_ps.jl.
############################################################
...
if !@isdefined(SOLVER_DIR);   const SOLVER_DIR   = joinpath(PROJECT_ROOT, "code", "solver_fct_ps"); end
if !@isdefined(SMM_DIR);      const SMM_DIR      = joinpath(PROJECT_ROOT, "code", "smm_fct_ps");    end
...
function load_all_smm_bundles!(;
    ...
    for w in ALL_WINDOWS
        jls_path = joinpath(smm_dir, "smm_result_$(w)$(suffix)_fct_ps.jls")
        if isfile(jls_path)
