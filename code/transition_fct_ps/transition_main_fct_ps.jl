############################################################
# transition_main.jl — Transition dynamics entry point
#                       (PS(x) = γ·x^{γ−1} variant)
#
# Usage (from project root):
#   julia --threads auto code/transition_fct_ps/transition_main.jl
#
...
#
# Workflow:
#   1. Load the two SMM result bundles (baseline + crisis) from smm_fct_ps
...
# Project layout:
#   code/
#     solver_fct_ps/   ← loaded as a library
#     smm_fct_ps/      ← smm_params.jl, smm.jl
#     transition_fct_ps/
#       transition_main.jl          ← this file
#       transition_params.jl
#       transition_solver.jl
#   output/
#     smm/                ← .jls estimation results  (smm_result_*_fct_ps.jls)
#     transition_fct_ps/  ← saved here
############################################################
...
# Must match the W_COND_TARGET that was set in code/smm_fct_ps/smm_main.jl.
#   0.0  →  "_diagonalW"
#   1.0  →  "_compressedW"
#   2.0  →  "_equalW"
#   >2.0 →  "_fullW"
...
println("="^65)
println("  Segmented Search Model — Transition Dynamics (PS(x) variant)")
println("="^65)
@printf("  Scenario:      %s\n", SCENARIO)
...
const SOLVER_DIR     = joinpath(PROJECT_ROOT, "code", "solver_fct_ps")
const SMM_DIR        = joinpath(PROJECT_ROOT, "code", "smm_fct_ps")
const OUTPUT_DIR     = joinpath(PROJECT_ROOT, "output")
const SMM_OUT_DIR    = joinpath(OUTPUT_DIR, "smm")
const TRANS_OUT_DIR  = joinpath(OUTPUT_DIR, "transition_fct_ps")
...
# ═══════════════════════════════════════════════════════════
# 6. Load transition modules
# ═══════════════════════════════════════════════════════════
print("Loading transition modules... "); flush(stdout)
include(joinpath(TRANSITION_DIR, "transition_params_fct_ps.jl"))
include(joinpath(TRANSITION_DIR, "transition_solver_fct_ps.jl"))
println("done."); flush(stdout)
...
# ═══════════════════════════════════════════════════════════
# 7. Determine windows & file paths
# ═══════════════════════════════════════════════════════════
...
# SMM result files carry the _fct_ps suffix (written by smm_fct_ps/smm_main.jl)
base_jls   = joinpath(SMM_OUT_DIR, "smm_result_$(base_window)$(W_SUFFIX)_fct_ps.jls")
crisis_jls = joinpath(SMM_OUT_DIR, "smm_result_$(crisis_window)$(W_SUFFIX)_fct_ps.jls")
...
# ═══════════════════════════════════════════════════════════
# 8. Load SMM bundles and reconstruct parameters
# ═══════════════════════════════════════════════════════════

function _load_bundle_or_error(path, label)
    isfile(path) || error("$label not found: $path\n" *
        "Run the SMM estimation first (code/smm_fct_ps/smm_main.jl).")
...
# ═══════════════════════════════════════════════════════════
# 9. Solve both stationary equilibria
# ═══════════════════════════════════════════════════════════
...
out_file = joinpath(TRANS_OUT_DIR, "transition_$(SCENARIO)$(W_SUFFIX)_fct_ps.jld2")
