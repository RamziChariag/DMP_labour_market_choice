...
#            <TRANS_OUT_DIR>/transition_<scenario><W>_fct_ps.jld2
#       6. Returns the absolute path of the saved file.
#
# Expects the following to be in scope (provided by transition_main.jl):
#   - all solver_fct_ps modules           (solve_model, SimParams, …)
#   - all smm_fct_ps modules              (_load_smm_bundle, unpack_θ, …)
#   - transition_params_fct_ps.jl         (TransitionParams)
#   - transition_solver_fct_ps.jl         (solve_transition)
...
the TransitionResult bundle to
    <TRANS_OUT_DIR>/transition_<scenario><W_SUFFIX>_fct_ps.jld2
...
function run_transition_simulation(
    ...
    base_jls   = joinpath(SMM_OUT_DIR,
                          "smm_result_$(base_window)$(w_suffix)_fct_ps.jls")
    crisis_jls = joinpath(SMM_OUT_DIR,
                          "smm_result_$(crisis_window)$(w_suffix)_fct_ps.jls")
...
if abspath(PROGRAM_FILE) == @__FILE__
    error("transition_simulation_fct_ps.jl is a library and cannot be run " *
          "standalone.  Use transition_main_fct_ps.jl as the entry point.")
end
