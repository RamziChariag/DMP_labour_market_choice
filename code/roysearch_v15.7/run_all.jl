...
"""
    run_one(script, window, opts, nthr, dry) -> (ok, seconds, logfile)
    ...
    try
        open(logf, "w") do io
            ...
            for line in eachline(proc)          # tee: durable log + live console
                println(io, line); println(line); flush(io); flush(stdout)
            end
            wait(proc)
            ok = success(proc)
        end
...
function main(argv)
    ...
    if concurrent
        ...
        for script in scripts
            tasks = [Threads.@spawn run_one(script, w, opts, per, dry) for w in windows]
