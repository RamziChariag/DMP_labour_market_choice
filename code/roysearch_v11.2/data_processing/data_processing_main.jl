...
#       sipp.jl                    — SIPP within-job wage-change rates
#       moments.jl     sigma.jl    — 30 moment targets + Σ̂, per window
...
#               j2j_ee_rates.csv, sipp_wchg_rates.csv, *_clean.arrow, ...
#
...
#   6b SIPP wchg rates     make_sipp_wchg()           → sipp_wchg_rates.csv
...
include(joinpath(PIPELINE_DIR, "sipp.jl"))         # make_sipp_wchg (SIPP wchg_rate_U/S)
...
_stage_banner("Stage 6b — SIPP within-job wage-change rates (wchg_rate_U, wchg_rate_S)")
make_sipp_wchg()
...
_stage_banner("Stage 9 — moment targets (30 moments × 4 windows; training_share × κ_w)")
all_moments = make_moments()

_stage_banner("Stage 10 — influence functions and Σ̂ (30×30 per window; ts row/col × κ_w)")
...
println("  • moments_{window}.csv         — 30 moments per window")
println("  • sigma_{window}.csv           — 30×30 variance-covariance matrix (full)")
...
println("  • sipp_wchg_rates.csv          — SIPP within-job wage-change hazards by window")
