...
#
# Combine step (Stage 7) — assemble all 30 empirical moments × 4 windows
...
# ──────────────────────────────────────────────────────────────────────────
# Stage 7 main: assemble all 30 moments × 4 windows
# ──────────────────────────────────────────────────────────────────────────
...
function make_moments()
    ...
    for (wname, wdef) in WINDOWS
        ...
        # wchg_rate_U / wchg_rate_S are DELIBERATELY absent here: sipp.jl
        # already returns the −log(1−p) hazard, so re-converting would apply
        # the transform twice (the ee_rate_S-style trap the recipe warns of).
