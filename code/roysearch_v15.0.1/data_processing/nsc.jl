...
#     adjustment for training_share (consumed by Stage 7 / Stage 10);
#   • _load_training_share_scale — reads κ_w back for the moment / sampling-
#     variance stages;
...
#
# The CPS training_share numerator is bounded by the SCHLCOLL question, whose
# age universe was 16–24 through 2012 and expanded to 16–54 in 2013. That break
# sits between the FC and COVID windows, so raw CPS training_share is not
# comparable across windows. NSC IPEDS-Universe counts all Title-IV
# postsecondary enrolments regardless of age, giving the age-comparable level.
#
# The CPS training_share moment is retained; NSC serves as a deterministic level
# anchor rather than a moment in its own right (NSC has only 3–5 Fall obs per
# window, too few to estimate a sampling variance). Per-window
#     κ_w  =  NSC_enr_w / CPS_enr_w
# rescales the training_share moment level. κ_w is treated as constant (NSC ≈
# census, ~90%+ IPEDS coverage), so the moment's sampling variance scales by κ_w².
#
# This routine:
...
#       Stage 10 (sampling variances) to consume.
#
# The CPS in_training flag excludes EDUC ≥ 111 (grad students), while NSC IPEDS
# Universe includes them, so κ absorbs the ~13% grad-student offset in addition
# to the age-universe expansion.
# ──────────────────────────────────────────────────────────────────────────
...
function compute_cps_nsc_scale()
    ...
    # Persist κ for Stage 7 (rescale training_share level) and Stage 10
    # (scale the training_share sampling variance by κ²).
    scale_df = DataFrame(
    ...
    @info "  Saved derived/training_share_scale.csv  (apply κ to training_share level + sampling variance)"
...
is non-finite or ≤ 0. Used by Stage 7 (training_share level) and Stage 10
(training_share sampling variance × κ²) to bake the NSC level adjustment
into the saved CSVs.
"""
