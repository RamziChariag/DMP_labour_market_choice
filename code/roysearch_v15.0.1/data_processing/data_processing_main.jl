...
const ROYSEARCH_VERSION = "15.0.1"
println("="^60)
...
# After every moment and σ̂²_samp file is written, re-read the FINAL derived
# products and check the cross-source identities that BIND in this model. Each
# line prints PASS/FAIL with the numeric gap. A FAIL is loud but NEVER crashes
# the pipeline; the check is wrapped so a genuinely structural break is reported,
# not swallowed.
#
# The non-binding relations (flow-balance ur = sep/(sep+jfr); wchg-vs-sep) are
# not tested — they do not hold as identities in this model.
