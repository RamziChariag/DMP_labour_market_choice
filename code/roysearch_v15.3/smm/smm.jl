...
function run_smm(
    ...
    elseif method in (:neldermead, :lbfgs, :bfgs)
        ...
        # Optim's g_abstol/x_abstol are ABSOLUTE, so on a Q of order 1e3–1e4 they
        # are unreachable and Optim.converged stays false even when the simplex
        # has stopped moving in any economically meaningful sense. Treat a run
        # that stalled on the relative scale of Q as converged, and say which
        # criterion fired so the log is not ambiguous.
        rel_stall = isfinite(best_loss[]) && best_loss[] != 0.0 &&
                    (iter_count[] - last_improve[]) >= min(r.nm_no_improve, 500)
